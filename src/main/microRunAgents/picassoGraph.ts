...
function systemPrompt(input: unknown): string {
  ...
  return [
    ...
    "Inspect the assigned graph before finalizing and support the result with that evidence. If graph access fails, return JSON with status blocked and the exact concrete failures in blockers; never present an uninspected design as completed.",
...
function validateOutput(output: unknown, toolCalls: MicroRunToolInvocation[], input: unknown): string | undefined {
  ...
  if (result.status === "blocked") {
    ...
    return blockerSummary ? undefined : "Picasso marked the run blocked without a concrete blocker.";
  }
...
function userMessage(input: unknown): string {
  ...
  if (task.mode === "assess") {
    return `Assess the assigned graph for this objective:\n\n${objective}\n\nChoose the evidence needed for a complete findings report. This is read-only analysis; do not submit graph operations.`;
  }
  return `Design the graph update for this objective:\n\n${objective}\n\nChoose the evidence and tactics needed for a coherent design, validate it, and submit any operations for review.`;
}
