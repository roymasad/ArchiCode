...
function isWriteCapableProvider(provider: ProjectSettings["providers"][number] | undefined): boolean {
  return (provider?.kind === "codex-local" || provider?.kind === "claude-local" || provider?.kind === "opencode-local") && provider.localSandbox !== "read-only";
}
...
function providerCommand(provider: ProjectSettings["providers"][number] | undefined): string | null {
  if (!provider || (provider.kind !== "codex-local" && provider.kind !== "claude-local" && provider.kind !== "opencode-local")) return null;
...
function isLocalProviderKind(kind: ProjectSettings["providers"][number]["kind"] | undefined): kind is "codex-local" | "claude-local" | "opencode-local" {
  return kind === "codex-local" || kind === "claude-local" || kind === "opencode-local";
}
...
async function pauseRunForDelphiSetupApproval(
  ...
  callId: string,
  input: { providerToolName: string; argumentsJson: string },
  continuation?: { providerKind: "codex-local" | "claude-local" | "opencode-local" | "api"; originalOutput: string }
): Promise<void> {
  const latest = await readRun(projectRoot, runId);
...
async function providerOptionsForRun(projectRoot: string, run: Run, settings: ProjectSettings, provider?: ProjectSettings["providers"][number]): Promise<{
  ...
  const localProvider = provider?.kind === "codex-local" || provider?.kind === "claude-local" || provider?.kind === "opencode-local";
  ...
  let codingRun = runSchema.parse({
    ...
    permission: {
      ...
      reason: reusablePolicy
        ? `Allowed by reusable policy ${reusablePolicy.id}.`
        : directWrite
          ? `${provider?.kind === "claude-local" ? "Claude Code CLI" : provider?.kind === "opencode-local" ? "OpenCode CLI" : "Codex Local CLI"} provider launch allowed by ${codexLocalSandboxDisplayLabel(provider?.localSandbox)}.`
