...
function isWriteCapableProvider(provider: ProjectSettings["providers"][number] | undefined): boolean {
  return (provider?.kind === "codex-local" || provider?.kind === "claude-local") && provider.localSandbox !== "read-only";
}
...
function providerCommand(provider: ProjectSettings["providers"][number] | undefined): string | null {
  if (!provider || (provider.kind !== "codex-local" && provider.kind !== "claude-local")) return null;
...
function isLocalProviderKind(kind: ProjectSettings["providers"][number]["kind"] | undefined): kind is "codex-local" | "claude-local" {
  return kind === "codex-local" || kind === "claude-local";
}
...
async function providerOptionsForRun(projectRoot: string, run: Run, settings: ProjectSettings, provider?: ProjectSettings["providers"][number]): Promise<{
  ...
  const localProvider = provider?.kind === "codex-local" || provider?.kind === "claude-local";
