...
const researchChangeSetJsonContract = [
  ...
  "        { \"kind\": \"start-agent-run\", \"flowId\": string, \"nodeId\": string, \"scope\": { \"kind\": \"project\" | \"flow\" | \"nodes\" | \"no-scope\", \"flowId\": string, \"nodeIds\": string[], \"label\": string }, \"promptSummary\": string, \"effort\": \"high\" | \"fast\", \"allowShell\": false, \"reusableApproval\": false, \"guidance\": { \"text\": string, \"evidence\": (\"last-error\" | \"trace-tail\" | \"latest-diff\" | \"runtime-log\" | \"node-notes\")[] } },",
  ...
  "When queueing AI Implement from Research, choose the run scope that best fits this specific implementation task, independent of the current chat scope. Use scope.kind \"project\" for broad app/project-wide work, \"flow\" for work centered on one flow, \"nodes\" for work centered on one or more concrete nodes, and \"no-scope\" only for trivial localized source edits that do not affect architecture, graph meaning, flow responsibilities, node acceptance criteria, data contracts, notes, or graph truth. Include nodeId as a primary anchor when useful for project/flow/nodes scopes, but scope.nodeIds is the multi-node focus. For no-scope, omit nodeId and keep scope.nodeIds empty. This scope is an internal handoff to the implementation agent; do not mention the chosen scope in the visible answer unless the user explicitly asks about implementation scoping.",
