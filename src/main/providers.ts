...
const researchGraphDelegationPolicy = [
  ...
  "Archi may prepare graph operations directly when the edit is simple, quick, tightly bounded, and its correct shape is already obvious without architecture or design synthesis. This can include a small set of explicitly named nodes plus obvious edges to an existing node, as well as one small rename, one short metadata correction, one note lifecycle change, one unambiguous node move, or one simple existing-node relationship.",
  "Any graph work that needs architecture or design synthesis requires Picasso. This includes creating or restructuring flows, decomposing a feature into many or semantically interdependent nodes, creating populated subflows, coordinating changes across several flows, deriving a graph from an attachment/specification, designing acceptance criteria and relationships together, or any broad architecture, refinement, reconciliation, or ambiguous multi-operation pass. These examples describe semantic complexity; operation count alone is not a Picasso trigger.",
  ...
  "If the user explicitly asks to propose, prepare, or show the review card directly and the requested scope is concrete, treat that request as confirmation. Inspect enough to validate the targets, then call the appropriate direct proposal tool or Picasso in the same turn without asking for a second confirmation.",
...
function researchSubagentDirective(options: ResearchProviderOptions): string {
  ...
  return [
    ...
    reconciliationEnabled
      ? `${researchGraphDelegationPolicy}\nFor read-only substantial graph analysis with no edits requested, call Picasso with mode assess; no graph-edit confirmation is needed and no change set is expected. For graph edits, inspect enough to describe the concrete, coherent scope. If the user explicitly asks to propose, prepare, or show the review card directly and the scope is concrete, that request is the confirmation: call the appropriate direct proposal tool or spawn_picasso in the same turn. Otherwise, end the visible response with a direct question asking whether the user wants that exact scope prepared as a review card. Picasso's graph changes are proposal-only and are not applied automatically. An explicit request to use Picasso does not replace confirmation when the requested scope itself is still ambiguous. If Sherlock already investigated the topic, pass only its compact evidence summary to Picasso.`
      : "",
    reconciliationEnabled
  ...
  for (const candidate of candidates) {
    try {
      ...
      const toolCalls = Array.isArray(record.toolCalls)
        ...
        : [];
      const answer = typeof record.answer === "string" && record.answer.trim() ? record.answer : undefined;
      if (!toolCalls.length && !answer) continue;
      return {
        answer,
        toolCalls
      };
...
export function localResearchToolLoopInstructions(options: ResearchProviderOptions): string {
  ...
  return [
    ...
    "When you need tools, return exactly one machine-readable JSON object, preferably in a fenced ```json block, with this top-level shape: { \"archicodeResearchTurn\": { \"answer\": string, \"toolCalls\": [{ \"id\": string, \"providerToolName\": string, \"arguments\": object | JSON-encoded-object-string }] } }.",
...
const researchChangeSetJsonContract = [
  ...
  "The archicodeResearch.answer field is the visible chat response. When the user asks to edit, create, update, delete, or move a graph item, inspect the affected nodes, edges, descriptions, acceptance criteria, and nearby graph context before proposing a change. State the concrete, coherent scope you intend to put on the review card, including related connections or node updates that should change together. If the user explicitly asks to propose, prepare, or show the review card directly and the scope is concrete, treat that request as confirmation and return the review-card changeSet after inspection in the same turn. Otherwise, ask every necessary clarifying question in that same response and ask once for confirmation; do not return a changeSet until the user affirms. For planning or brainstorming, answer in normal prose without a changeSet.",
  ...
  `When proposing ${gaiaAgent.name} through a start-agent-run approval card, make the visible answer outcome-oriented for a new user: say that you prepared an AI implementation approval and that approving it will start an implementation run that can create or modify project files for the described scope, with progress and results visible in Activity. You may name ${gaiaAgent.name} secondarily, but never use "Queueing ${gaiaAgent.name}" or "Queue ${gaiaAgent.name}" as the only explanation of what happens next.`,
  ...
  "{",
  "  \"archicodeResearch\": {",
  "    \"answer\": \"I prepared an AI implementation approval for the selected node. Approving it will start an implementation run that can create or modify the relevant project files; progress and results will appear in Activity.\",",
  "    \"summary\": \"Prepared an AI implementation approval for the selected node.\",",
  "    \"changeSet\": {",
  "      \"summary\": \"Implement the selected node\",",
  "      \"operations\": [",
  "        {",
...
const researchSystemPrompt = [
  ...
  "If the user asks you to edit, update, create, delete, move, or otherwise change the graph, first inspect/research enough to understand the affected nodes, edges, descriptions, acceptance criteria, and adjacent responsibilities. Propose a coherent graph change rather than an isolated edit: include related connection, description, or criterion updates when they are needed to keep the graph truthful. When the user explicitly asks to propose, prepare, or show the review card directly and the scope is concrete, treat that request as confirmation and provide the card after inspection in the same turn. Otherwise, ask every clarification you need in the same scope-confirmation response, then ask exactly once whether this is the scope they want prepared as a review card. For planning, brainstorming, or an unspecified improvement, discuss the direction without preparing a card until the user requests a concrete edit.",
  ...
  "Do not say you cannot edit the graph merely because direct application is gated. For a concrete request to propose/prepare/show the card, inspect and provide the pending review card in the same turn; otherwise confirm the scope once before providing it. The card's Apply/Reject controls or auto-approve setting decide whether it is applied.",
  "Do not include a changeSet for a vague request to propose, plan, brainstorm, or improve. A concrete request to change a known graph item or create described items is enough to inspect and, when the user explicitly asks for the card directly, prepare it without another confirmation.",
