...
async function requireCommit(projectRoot: string, revision: string): Promise<string> {
  ...
  if (!candidate) throw new Error("Choose both branches before previewing graph changes.");
  const result = await runGit(projectRoot, ["rev-parse", "--verify", "--end-of-options", `${candidate}^{commit}`]);
  if (!result.ok) throw new Error(`Git branch or revision "${candidate}" is not available.`);
...
async function readGraphFlows(projectRoot: string, commit: string): Promise<Flow[]> {
  ...
  if (paths.length > MAX_GRAPH_PREVIEW_FLOWS) {
    throw new Error(`This graph contains more than ${MAX_GRAPH_PREVIEW_FLOWS} flows and is too large to preview safely.`);
  }
  const snapshots = await Promise.all(paths.map(async (filePath) => {
    const text = await showFile(projectRoot, commit, filePath);
    if (!text) throw new Error(`Unable to read committed graph flow ${filePath}.`);
    try {
      ...
      throw new Error(`Committed graph flow ${filePath} is not valid ArchiCode data.`);
    }
  }));
  return snapshots;
}
...
export async function previewGraphBranches(
  ...
  if (!mergeBase.ok || !mergeBase.stdout.trim()) {
    throw new Error("These branches do not share a Git ancestor, so a PR-style graph preview is unavailable.");
  }
