...
export function CodeKnowledgeMapView({ flow, searchQuery, onSelectNode, onSelectEdge, onOpenSource, refreshState, onRefresh, codeDetailAvailable, onOpenCodeDetail }: {
  ...
  const communities = useMemo<CommunityMeta[]>(() => {
    ...
    return [...members].map(([id, nodes]) => {
      if (id.startsWith("group:")) {
        ...
        return { id, label, description: t("An explicit architecture group from the main canvas."), count: nodes.length };
      }
      if (id.startsWith("scope:")) {
        ...
        return { id, label, description: t("Concepts that belong to the same architecture detail flow."), count: nodes.length };
      }
      if (id.startsWith("dependency:")) {
        ...
        return { id, label, description: t("A detected relationship cluster {{value1}}.", { value1: representative ? ` centered on ${representative.title}` : "" }), count: nodes.length };
      }
      const type = id.slice("type:".length);
      return { id, label: t("{{value1}} concepts", { value1: titleCase(type) }), description: t("Concepts grouped by their architecture type because no stronger relationship cluster was available."), count: nodes.length };
  ...
  return <div ref={rootRef} className="knowledge-map-view" aria-label={t("Architecture lens map for {{name}}", { name: flow.name })} style={{ "--knowledge-map-sidebar-width": `${sidebarWidth}px` } as CSSProperties}>
    <div className="knowledge-map-toolbar">
      <div><strong>{t("Architecture Lens Map")}</strong><small>{t("{{name}} · {{length}} concepts · {{length2}} relationships · {{length3}} communities", { name: flow.name, length: flow.nodes.length, length2: flow.edges.length, length3: communities.length })}</small></div>
      <label><Search size={15} /><input value={localQuery} onChange={(event) => setLocalQuery(event.target.value)} placeholder={t("Search concepts, communities, relationships, or source paths…")} />{localQuery ? <button type="button" onClick={() => setLocalQuery("")} aria-label={t("Clear knowledge map search")}><X size={14} /></button> : null}</label>
      <Tooltip content={t("Explain dots, lines, colors, highlighting, and search.")}><button type="button" className={`knowledge-map-reset${showGuide ? " is-active" : ""}`} aria-label={t("Explain this map")} aria-pressed={showGuide} onClick={() => setShowGuide((current) => !current)}><CircleHelp size={15} /></button></Tooltip>
      <Tooltip content={t("Return to the complete map at its default zoom.")}><button type="button" className="knowledge-map-reset" aria-label={t("Center and reset zoom")} onClick={() => setTransform({ x: 0, y: 0, scale: 1 })}><Crosshair size={15} /></button></Tooltip>
      <Tooltip content={t("Re-scan local source and verify whether imported relationships are still supported.")}><button type="button" className="knowledge-map-reset" aria-label={t("Refresh relationship evidence")} disabled={refreshState.status === "refreshing"} onClick={onRefresh}><RefreshCw size={15} className={refreshState.status === "refreshing" ? "is-spinning" : ""} /></button></Tooltip>
      <Tooltip content={codeDetailAvailable ? t("Open the project-wide file, symbol, import, call, and runtime knowledge map.") : t("Refresh evidence first to build the project-wide code knowledge map.")}><button type="button" className="knowledge-map-reset" disabled={!codeDetailAvailable} aria-label={t("Open project-wide code knowledge map")} onClick={onOpenCodeDetail}><Braces size={15} /></button></Tooltip>
    </div>
    <div className="knowledge-map-search-summary">{query ? t("{{size}} concepts and {{size2}} relationships match", { size: matchingNodeIds.size, size2: matchingEdgeIds.size }) : t("Search includes concept names, descriptions, community names, relationship types, and source evidence.")}</div>
    ...
    <aside className="knowledge-map-sidebar">
      {selectedNode ? <section className="knowledge-map-detail knowledge-map-primary-detail"><span className="knowledge-map-kicker">{t("Concept")}</span><h3>{selectedNode.title}</h3><p>{selectedNode.description || t("No concept description has been added yet.")}</p><small>{t("{{type}} · {{value2}}", { type: selectedNode.type, value2: focusedCommunityMeta?.label ?? "Other concepts" })}</small><small><strong>{directRelationships}</strong> {" "}{t("directly connected relationships are highlighted; the rest of this community has a colored halo.")}</small>{selectedImpact ? <small>{t("{{value1}} potential dependents {{value2}}.", { value1: Math.max(0, selectedImpact.nodeIds.length - 1), value2: selectedImpact.truncated ? " (bounded preview)" : "" })}</small> : null}</section> : null}
      {!selectedNode && !selectedEdge && focusedCommunityMeta ? <section className="knowledge-map-detail knowledge-map-primary-detail"><span className="knowledge-map-kicker">{t("Community focus")}</span><h3>{focusedCommunityMeta.label}</h3><p>{focusedCommunityMeta.description}</p><small>{t("{{count}} concepts highlighted; other visible communities are softened.", { count: focusedCommunityMeta.count })}</small></section> : null}
      {selectedEdge ? <section className="knowledge-map-detail knowledge-map-primary-detail"><span className="knowledge-map-kicker">{t("Relationship evidence")}</span><h3>{selectedEdge.label || t("Relationship")}</h3><p>{t("{{title}} → {{title2}}", { title: nodeById.get(selectedEdge.source)?.title, title2: nodeById.get(selectedEdge.target)?.title })}</p><div className="knowledge-map-evidence-badges"><span>{selectedEdge.evidence?.origin ?? t("unverified")}</span><span>{t("{{value1}}%", { value1: Math.round((selectedEdge.evidence?.confidence ?? 0) * 100) })}</span><span>{selectedEdge.evidence?.verification ?? t("unresolved")}</span><span>{selectedEdge.evidence?.freshness ?? t("unknown")}</span></div>{selectedEdge.evidence?.relationKinds.length ? <p>{selectedEdge.evidence.relationKinds.join(" · ")}</p> : null}{selectedEdge.evidence?.locations.map((location, index) => <button type="button" className="knowledge-map-source" key={`${location.path}:${location.line ?? index}`} onClick={() => onOpenSource(location.path, location.line)}><FileCode2 size={14} /><span>{t("{{path}} {{value2}}", { path: location.path, value2: location.line ? `:${location.line}` : "" })}</span></button>)}</section> : null}
      {(showGuide || (!selectedNode && !selectedEdge && !focusedCommunity)) ? <section className="knowledge-map-guide"><span className="knowledge-map-kicker">{t("How to read this lens")}</span><h3>{t("Architecture concepts in the selected perspective")}</h3><p><strong>{t("Dots")}</strong> {" "}{t("are architecture concepts in")}{" "}<strong>{flow.name}</strong>. <strong>{t("Lines")}</strong> {" "}{t("are relationships.")}{" "}<strong>{t("Colors")}</strong> {" "}{t("group concepts that belong together.")}</p><ul><li>{t("Changing the top-level flow changes this architecture lens.")}</li><li>{t("Hover for a name; select for its full description.")}</li><li>{t("Solid lines are extracted or resolved; dashed lines are inferred.")}</li><li>{t("Open Code Knowledge Map for project-wide files, symbols, imports, calls, and runtime links.")}</li></ul></section> : null}
