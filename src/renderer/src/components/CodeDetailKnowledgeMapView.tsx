...
export function CodeDetailKnowledgeMapView({ snapshot, refreshState, onRefresh, onBack, onOpenSource, onSelectArchitectureNode }: {
  ...
  const communityMeta = useMemo<CommunityMeta[]>(() => snapshot.communities.filter((community) => community.nodeCount > 0).map((community) => {
    ...
    if (/^community-\d+$/i.test(community.label) || /^community-\d+$/i.test(community.id)) {
      ...
      return { id: community.id, label: representative ? `Around ${representative.label}` : "Related code", count: community.nodeCount, description: t("A detected file/symbol relationship cluster {{value1}}.", { value1: representative ? ` centered on ${representative.path}` : "" }) };
    }
    return { id: community.id, label: titleCase(community.label), count: community.nodeCount, description: t("Files and symbols assigned to the same architecture area.") };
  ...
  return <div ref={rootRef} className="knowledge-map-view code-detail-map" aria-label={t("Project-wide code knowledge map")} style={{ "--knowledge-map-sidebar-width": `${sidebarWidth}px` } as CSSProperties}>
    <div className="knowledge-map-toolbar"><div><strong>{t("Code Knowledge Map")}</strong><small>{t("Whole project · {{files}} files · {{symbols}} symbols · {{dependencies}} dependencies · {{calls}} resolved calls", { files: snapshot.stats.files, symbols: snapshot.stats.symbols, dependencies: snapshot.stats.dependencies, calls: snapshot.stats.calls })}</small></div><label><Search size={15} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t("Search files, symbols, languages, or communities…")} />{query ? <button type="button" onClick={() => setQuery("")} aria-label={t("Clear code knowledge search")}><X size={14} /></button> : null}</label><Tooltip content={t("Explain files, symbols, relationships, quality, impact, and path tracing.")}><button type="button" className={`knowledge-map-reset${showGuide ? " is-active" : ""}`} aria-label={t("Explain this code map")} aria-pressed={showGuide} onClick={() => setShowGuide((current) => !current)}><CircleHelp size={15} /></button></Tooltip><Tooltip content={t("Return to the complete map at its default zoom.")}><button type="button" className="knowledge-map-reset" aria-label={t("Center and reset zoom")} onClick={() => setTransform({ x: 0, y: 0, scale: 1 })}><Crosshair size={15} /></button></Tooltip><Tooltip content={t("Re-scan the project and rebuild local file, symbol, dependency, call, and runtime evidence.")}><button type="button" className="knowledge-map-reset" aria-label={t("Refresh code knowledge")} disabled={refreshState.status === "refreshing"} onClick={onRefresh}><RefreshCw size={15} className={refreshState.status === "refreshing" ? "is-spinning" : ""} /></button></Tooltip><Tooltip content={t("Return to the selected architecture lens.")}><button type="button" className="knowledge-map-reset" aria-label={t("Return to architecture lens map")} onClick={onBack}><ArrowLeft size={15} /></button></Tooltip></div>
    <div className="knowledge-map-search-summary">{normalizedQuery ? t("{{length}} matching files or symbols shown", { length: renderedNodes.length }) : t("Search matches file paths, symbol names, languages, kinds, and community names.")}</div>
    ...
    <div className="knowledge-map-sidebar-resizer" role="separator" aria-label={t("Resize knowledge map sidebar")} aria-orientation="vertical" tabIndex={0} onPointerDown={startSidebarResize} onKeyDown={adjustSidebarWidth} />
    <aside className="knowledge-map-sidebar">
      {selectedNode ? <section className="knowledge-map-detail knowledge-map-primary-detail"><span className="knowledge-map-kicker">{t("{{kind}} {{value2}}", { kind: selectedNode.kind, value2: selectedNode.symbolKind ? ` · ${selectedNode.symbolKind}` : "" })}</span><h3>{selectedNode.label}</h3><p>{t("{{path}} {{value2}}", { path: selectedNode.path, value2: selectedNode.line ? `:${selectedNode.line}` : "" })}</p><small>{t("{{community}} is highlighted; connected lines show how this item participates in the code graph.", { community: communityById.get(selectedNode.community)?.label ?? selectedNode.community })}</small><button type="button" className="knowledge-map-source" onClick={() => onOpenSource(selectedNode.path, selectedNode.line)}><FileCode2 size={14} /> {" "}{t("Open source")}</button>{selectedNode.architectureNodeId ? <button type="button" className="knowledge-map-source" onClick={() => onSelectArchitectureNode(selectedNode.architectureNodeId!)}><Network size={14} /> {" "}{t("Open architecture concept")}</button> : null}<button type="button" className="knowledge-map-source" onClick={() => { setImpactRootId(impactRootId === selectedNode.id ? null : selectedNode.id); setPathResult(null); }}><Activity size={14} /> {impactRootId === selectedNode.id ? t("Clear impact") : t("Show potential impact")}</button><button type="button" className="knowledge-map-source" onClick={() => { setPathStartId(pathStartId === selectedNode.id ? null : selectedNode.id); setPathResult(null); }}><Route size={14} /> {pathStartId === selectedNode.id ? t("Clear path start") : pathStartId ? t("Replace path start") : t("Start path here")}</button>{pathStartId && pathStartId !== selectedNode.id ? <small>{t("{{value1}} from {{value2}}.", { value1: pathResult ? `${pathResult.edgeIds.length} relationship path` : "No bounded path found", value2: nodeById.get(pathStartId)?.label ?? "start" })}</small> : null}</section> : null}
      {selectedEdge ? <section className="knowledge-map-detail knowledge-map-primary-detail"><span className="knowledge-map-kicker">{selectedEdge.kind}</span><h3>{selectedEdge.evidence.relationKinds.join(" · ") || selectedEdge.kind}</h3><p>{t("{{label}} → {{label2}}", { label: nodeById.get(selectedEdge.source)?.label, label2: nodeById.get(selectedEdge.target)?.label })}</p><div className="knowledge-map-evidence-badges"><span>{selectedEdge.evidence.origin}</span><span>{t("{{value1}}%", { value1: Math.round(selectedEdge.evidence.confidence * 100) })}</span><span>{selectedEdge.evidence.verification}</span></div>{selectedEdge.evidence.locations.map((location, index) => <button type="button" className="knowledge-map-source" key={`${location.path}:${location.line ?? index}`} onClick={() => onOpenSource(location.path, location.line)}><FileCode2 size={14} />{location.path}{location.line ? t(": {{line}}", { line: location.line }) : ""}</button>)}</section> : null}
      {!selectedNode && !selectedEdge && focusedCommunityMeta ? <section className="knowledge-map-detail knowledge-map-primary-detail"><span className="knowledge-map-kicker">{t("Community focus")}</span><h3>{focusedCommunityMeta.label}</h3><p>{focusedCommunityMeta.description}</p><small>{t("{{count}} files or symbols highlighted; other visible communities are softened.", { count: focusedCommunityMeta.count })}</small></section> : null}
      {(showGuide || (!selectedNode && !selectedEdge && !focusedCommunity)) ? <section className="knowledge-map-guide"><span className="knowledge-map-kicker">{t("How to read code knowledge")}</span><h3>{t("Project-wide files, symbols, and structural relationships")}</h3><p><strong>{t("Larger dots")}</strong> {" "}{t("are files;")}{" "}<strong>{t("smaller dots")}</strong> {" "}{t("are symbols. Lines represent containment, imports, resolved calls, or runtime links.")}</p><ul><li>{t("This map covers the project rather than only the selected architecture lens.")}</li><li>{t("Hover for identity and source path.")}</li><li>{t("Select an item to open source, trace impact, or start a path.")}</li><li>{t("The full snapshot remains local and agents query only bounded results.")}</li></ul></section> : null}
      <section className="code-detail-quality"><h3>{t("Truth quality")}</h3><div><span>{t("Import resolution")}</span><strong>{t("{{value1}}%", { value1: Math.round(snapshot.stats.resolutionRate * 100) })}</strong></div><div><span>{t("Unresolved imports")}</span><strong>{snapshot.stats.unresolvedImports}</strong></div><div><span>{t("Snapshot")}</span><strong>{snapshot.stats.truncated ? t("bounded") : t("complete")}</strong></div>{snapshot.stats.truncated ? <small>{t("Showing a safe local cap of {{length}} nodes and {{length2}} edges.", { length: snapshot.nodes.length, length2: snapshot.edges.length })}</small> : null}</section>
      <section className="knowledge-map-communities"><button type="button" className="knowledge-map-section-toggle" aria-expanded={communitiesExpanded} onClick={() => setCommunitiesExpanded((current) => !current)}>{communitiesExpanded ? <ChevronDown size={15} /> : <ChevronRight size={15} />}<span><strong>{t("Communities")}</strong><small>{t("Eye controls visibility; name focuses a community")}</small></span></button>{communitiesExpanded ? <><label className="knowledge-map-community-search"><Search size={13} /><input value={communityQuery} onChange={(event) => setCommunityQuery(event.target.value)} placeholder={t("Search communities…")} />{communityQuery ? <button type="button" aria-label={t("Clear community search")} onClick={() => setCommunityQuery("")}><X size={12} /></button> : null}</label><div className="knowledge-map-filter-actions"><button type="button" onClick={showAllCommunities}>{t("Show all")}</button><button type="button" onClick={hideAllCommunities}>{t("Hide all")}</button></div><div className="knowledge-map-community-list">{filteredCommunities.map((community) => { const visible = !hiddenCommunities.has(community.id); const focused = community.id === focusedCommunity; return <div key={community.id} className={`knowledge-map-community-row${focused ? " is-selected-community" : ""}${visible ? "" : " is-hidden-community"}`}><Tooltip content={t("{{value1}}{{label}}", { value1: visible ? "Hide" : "Show", label: community.label })}><button type="button" className="knowledge-map-community-eye" aria-label={t("{{value1}} community {{label}}", { value1: visible ? "Hide" : "Show", label: community.label })} aria-pressed={visible} onClick={() => toggleCommunityVisibility(community.id)}>{visible ? <Eye size={13} /> : <EyeOff size={13} />}</button></Tooltip><span className="knowledge-map-community-color" style={{ backgroundColor: colorByCommunity.get(community.id) }} /><Tooltip content={community.description}><button type="button" className="knowledge-map-community-name" aria-label={t("Focus community {{label}}", { label: community.label })} aria-pressed={focused} onClick={() => focusCommunity(community.id)}><strong>{community.label}</strong><small>{community.count}</small></button></Tooltip></div>; })}</div></> : null}</section>
    </aside>
    <div className="code-detail-render-limit">{t("Rendering {{length}} / {{length2}} nodes · {{length3}} / {{length4}} edges", { length: renderedNodes.length, length2: snapshot.nodes.length, length3: renderedEdges.length, length4: snapshot.edges.length })}</div>
  </div>;
}
