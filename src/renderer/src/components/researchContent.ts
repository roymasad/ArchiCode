...
export type GraphReviewReportPresentation = {
  summary: string;
  details: string;
  operationCount: number;
};

export function graphReviewReportPresentation(content: string): GraphReviewReportPresentation | null {
  const trimmed = content.trim();
  if (!trimmed.startsWith("Graph review complete for ")) return null;
  const detailsMatch = trimmed.match(/\n\n(?=(?:Applied|Rejected|Failed):\s)/);
  ...
  const outcome = header.match(/(?:^|\n\n)(\d+ applied,\s+\d+ rejected,\s+\d+ failed)\.?$/)?.[1];
  if (!outcome || !details) return null;
  return {
    summary: `${outcome}.`,
    details,
    operationCount: details.match(/^(?:Applied|Rejected|Failed):\s/gm)?.length ?? 0
  };
}
